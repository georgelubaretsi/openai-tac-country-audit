...
export function enrichOfficialComparison(comparison, populationEnrichment) {
  ...
  return {
    ...comparison,
    schema: comparison.schema === "openai-cyber-verification-country-support/official-access-comparison/v3"
      ? comparison.schema
      : "openai-cyber-verification-country-support/official-access-comparison/v2",
    population_source: {
      label: POPULATION_SOURCE_LABEL,
      reference_year: POPULATION_YEAR,
...
export function officialComparisonCsv(comparison) {
  return [
    "index,code,iso3,name,population,population_year,population_status,official_source_name,official_chatgpt_supported,cyber_verification_supported,classification,active_us_sanctions_program_ids",
    ...comparison.results.map(result => [
      ...
      csvCell((result.active_us_sanctions_program_ids ?? []).join(";")),
    ].join(",")),
  ].join("\n") + "\n";
}
